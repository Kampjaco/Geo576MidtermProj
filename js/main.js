...
require([
  ...
  // Add Legend widget in the bottom left corner
  ...
  clearFiltersButton.innerHTML = `<span class="esri-icon-erase"></span>`;
  clearFiltersButton.title = "Clear Selection";
  clearFiltersButton.style.border = "none";
  clearFiltersButton.style.background = "transparent";
  clearFiltersButton.addEventListener("click", function () {
    // Clear selected filters
    selectedFilters.trip_name = null;
    selectedFilters.logged_by = null;
    selectedFilters.date_visited = null;

    // Remove selection style from all options
    const optionItems = filtersDiv.querySelectorAll(".optionItem");
    optionItems.forEach((item) => item.classList.remove("selectedFilter"));

    // Apply filters with cleared selections
    applyFilters();
  });
  filtersDiv.appendChild(clearFiltersButton);
